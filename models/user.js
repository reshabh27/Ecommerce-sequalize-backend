...
module.exports = (sequelize, DataTypes) => {
    ...
    User.prototype.generateToken = async function () {
        const user = this;
        const token = jwt.sign({ id: user.id.toString(), userType: user.role }, "secret_secret");
    ...
    //Middleware to delete associated book history of user 
    //   User.beforeDestroy(async (user, options) => {

    //     try {
    //       // Delete associated books boorrowed history
    //       await userbook.destroy({ where: { userId: user.id } });

    //       // Delete associated transaction
    //       await Budget.destroy({ where: { userId: user.id } });
    //     } catch (error) {
    //       console.error('Error deleting associated books and transactions:', error);
    //     }
    //   });
